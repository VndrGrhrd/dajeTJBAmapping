...
class Utils {
    ...
    async setFieldValueTyping(field, value) {
        await browser.execute('$("#ajuda").fadeOut();')
    ...
    async selectByValue(field, value) {
        try {
            ...
            await browser.execute(`$('${field} option[value*="${value}"]').attr('selected','selected')`)
            await browser.execute(`$('${field}').val('${value}').change()`)
        }
    }
    ...
    async waitForExisting(field, timeOut = 20000) {
       try {
        await browser.execute('$("#ajuda").fadeOut();')
        return await browser.$(field).waitForExist({timeout: timeOut || TIME_TO_WAIT})
       } catch (error) {
        return false
       }
    }
    ...
    async checkVisibity(field) {
       ...
       return await browser.$(field).isDisplayed()
        await browser.$(field).scrollIntoView({ block: 'center' })
    }

    async waitForVisible(field) {
    return    await browser.$(field).waitForDisplayed(TIMEOUT_WIO)
        await browser.$(field).scrollIntoView({ block: 'center' })
    }
